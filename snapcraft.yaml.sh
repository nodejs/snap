...
while getopts "r:g:" opt; do
  case $opt in
    r)
      ...
      # release
      NODE_VERSION="$(curl -sL --show-error --fail https://nodejs.org/download/release/index.tab | tac | tac | awk '/^v'"$OPTARG"'\..*[^a-z0-9]src[^a-z0-9]/ { print substr($1, 2); exit }')"
...
if [ -z ${NODE_DISTTYPE+x} ]; then
  # nightly
  NODE_VERSION="$(curl -sL --show-error --fail https://nodejs.org/download/nightly/index.tab | tac | tac | awk '/^v[1-9].*[^a-z0-9]src[^a-z0-9]/ { print substr($1, 2); exit }')"
